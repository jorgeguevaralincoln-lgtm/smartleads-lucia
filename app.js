...
/** Ajusta si cambias el admin en rules */
...
// SIN MEMORIA (sesion nueva al recargar)
...
if (!useLocalStorage) {
  onAuthStateChanged(auth, (user) => {
    if (user) {
      ...
      // ✅ FIX: Guardar userId desde el inicio (para tus Rules)
      if (user.isAnonymous) {
        try {
          setDoc(
...
window.openLoginModal = () => {
  // si ya está logueado (no anónimo), entra directo
  if (currentUser && !currentUser.isAnonymous) {
    switchView("admin");
    return;
  }
  // si es anónimo, mostrar modal
...
window.performLogin = async () => {
  ...
  if (useLocalStorage) {
    if (pass === "123456") {
      ...
      errorMsg.innerText = "Modo Local: 123456";
...
window.savePrompt = async () => {
  ...
  try {
    ...
    window.showToast("No se pudo guardar en la nube (Permisos/Conexión)");
  }
};
...
window.openDetailModal = async (id) => {
  ...
  if (!data) {
    if (useLocalStorage) data = JSON.parse(localStorage.getItem("sl_leads_data") || "{}")[id];
    else {
  ...
  if (data) {
    document.getElementById("detail-summary").innerText = data.summary || "Esperando análisis...";
      ...
      document.getElementById("detail-history").innerHTML = Object.values(data.history)
        ...
        .map(
          (m) =>
            ...
            }">${m.role === "lucia" ? "Lucía" : "Cliente"}:</span> ${m.text}</div>`
        )
        .join("");
    }
...
async function save(txt, role, status = null) {
  const data = {
    userId: currentUser?.uid, // ✅ FIX: para rules
    lastMessage: txt,
...
function appendMessageBubble(role, text, shouldSave = true) {
  ...
  if (isClosing) {
    ...
    b.onclick = () => window.open(`https://wa.me/${JORGE_PHONE}?text=Hola Jorge, acabo de hablar con Lucía...`);
...
async function sendMessage(manualText = null) {
  const text = manualText || document.getElementById("user-input").value.trim();
  if (!text) return;
...
function getLocalLeads() {
  return Object.values(JSON.parse(localStorage.getItem("sl_leads_data") || "{}")).sort(
